/**
 * Usage Tracking Middleware for Law Analytics Service
 *
 ...
 *
 * Law-specific operations: usage, performance, cost, metrics
 */
...
const SERVICE_NAME = 'law-analytics';
...
// ============================================================================
// Operation Detection - Analytics-Specific
// ============================================================================
...
function detectOperation(req: Request): string {
  ...
  if (path.includes('/usage/summary')) return 'usage_summary';
  if (path.includes('/usage/breakdown')) return 'usage_breakdown';
  if (path.includes('/usage/trends')) return 'usage_trends';
  if (path.includes('/usage/export')) return 'usage_export';
  if (path.includes('/usage/by-user')) return 'usage_by_user';
  if (path.includes('/usage/by-organization')) return 'usage_by_organization';
  if (path.includes('/usage')) return 'usage_general';

  // Analytics - performance operations
  if (path.includes('/performance/latency')) return 'performance_latency';
  if (path.includes('/performance/throughput')) return 'performance_throughput';
  if (path.includes('/performance/errors')) return 'performance_errors';
  if (path.includes('/performance/overview')) return 'performance_overview';
  if (path.includes('/performance/trends')) return 'performance_trends';
  if (path.includes('/performance')) return 'performance_general';

  // Analytics - cost operations
  if (path.includes('/cost/summary')) return 'cost_summary';
  if (path.includes('/cost/breakdown')) return 'cost_breakdown';
  if (path.includes('/cost/forecast')) return 'cost_forecast';
  if (path.includes('/cost/savings')) return 'cost_savings';
  if (path.includes('/cost/by-service')) return 'cost_by_service';
  if (path.includes('/cost/export')) return 'cost_export';
  if (path.includes('/cost')) return 'cost_general';

  // Analytics - metrics operations
  if (path.includes('/metrics/realtime')) return 'metrics_realtime';
  if (path.includes('/metrics/historical')) return 'metrics_historical';
  if (path.includes('/metrics/aggregate')) return 'metrics_aggregate';
  if (path.includes('/metrics/dashboard')) return 'metrics_dashboard';
  if (path.includes('/metrics/custom')) return 'metrics_custom';
  if (path.includes('/metrics')) return 'metrics_general';
