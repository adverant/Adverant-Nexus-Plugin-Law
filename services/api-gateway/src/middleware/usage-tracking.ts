/**
 * Usage Tracking Middleware for Law API Gateway
 *
 ...
 *
 * Law-specific operations: queries, documents, research, citations, auth
 */
...
const SERVICE_NAME = 'law-api-gateway';
...
// ============================================================================
// Operation Detection - Law-Specific
// ============================================================================
...
function detectOperation(req: Request): string {
  ...
  // Law API Gateway operations - queries
  if (path.includes('/queries/natural-language')) return 'queries_natural_language';
  if (path.includes('/queries/structured')) return 'queries_structured';
  if (path.includes('/queries/semantic')) return 'queries_semantic';
  if (path.includes('/queries')) return 'queries_general';

  // Law API Gateway operations - documents
  if (path.includes('/documents/upload')) return 'documents_upload';
  if (path.includes('/documents/batch')) return 'documents_batch';
  if (path.includes('/documents/analyze')) return 'documents_analyze';
  if (path.includes('/documents/dna')) return 'documents_dna';
  if (path.includes('/documents/extract')) return 'documents_extract';
  if (path.includes('/documents')) return 'documents_general';

  // Law API Gateway operations - research
  if (path.includes('/research/create')) return 'research_create';
  if (path.includes('/research/analyze')) return 'research_analyze';
  if (path.includes('/research/memo')) return 'research_memo';
  if (path.includes('/research/findings')) return 'research_findings';
  if (path.includes('/research/stream')) return 'research_stream';
  if (path.includes('/research')) return 'research_general';

  // Law API Gateway operations - citations
  if (path.includes('/citations/validate')) return 'citations_validate';
  if (path.includes('/citations/graph')) return 'citations_graph';
  if (path.includes('/citations/network')) return 'citations_network';
  if (path.includes('/citations/analyze')) return 'citations_analyze';
  if (path.includes('/citations/extract')) return 'citations_extract';
  if (path.includes('/citations')) return 'citations_general';
  ...
  if (path.includes('/auth/verify')) return 'auth_verify';
  if (path.includes('/auth')) return 'auth_general';

  // Streaming operations
  if (path.includes('/stream')) return 'streaming';
  if (path.includes('/ws')) return 'websocket';
...
export function usageTrackingMiddleware(
  ...
  res.on('finish', () => {
    ...
    const report: UsageReport = {
      ...
      organizationId: req.user?.organizationId || req.tenantContext?.companyId,
